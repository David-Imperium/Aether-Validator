...
pub fn show_memory_context(path: &Path) -> Result<()> {
    use aether_intelligence::MemoryStore;
...
pub fn show_intent_analysis(path: &Path) -> Result<()> {
    use aether_intelligence::IntentInferrer;
...
fn to_violation_record(v: &ViolationInfo) -> ViolationRecord {
    ViolationRecord {
        ...
        severity: match v.severity {
            aether_validation::Severity::Critical => IntelSeverity::Error,
            aether_validation::Severity::Error => IntelSeverity::Error,
            aether_validation::Severity::Warning => IntelSeverity::Warning,
            aether_validation::Severity::Info => IntelSeverity::Info,
            aether_validation::Severity::Hint => IntelSeverity::Style,
        },
        line: v.line,
        column: v.column,
...
pub fn handle_state_and_accept(
    ...
    if options.save_state {
        let state_path = aether_dir.join("validation_state");
        if let Ok(mut state) = ValidationState::new(Some(state_path)) {
            ...
            if let Err(e) = state.save_project(&project) {
                ...
                println!("\n✓ Validation state saved to .aether/validation_state/");
            }
        }
    }
    ...
    if let Some(ref accept_ids) = options.accept_ids {
        if let Some(ref reason_text) = options.reason {
            ...
            let config = Config {
                decision_log_path: Some(aether_dir.join("decision_log.json")),
                validation_state_path: Some(aether_dir.join("validation_state")),
                code_graph_path: Some(aether_dir.join("code_graph.json")),
                ..Config::default()
            };

            if let Ok(mut ai) = AetherIntelligence::new(config) {
