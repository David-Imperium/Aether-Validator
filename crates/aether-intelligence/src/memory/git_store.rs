...
//! changes to the .aether/ directory in git.
...
impl GitMemoryStore {
    ...
    pub fn new(store: MemoryStore, project_root: PathBuf, auto_commit: bool) -> Self {
        let aether_path = MemoryPath::project_base(&project_root);
        Self {
            store,
            project_root,
            auto_commit,
    ...
    /// Add .aether/ to git staging
    fn add_aether(&self) -> Result<()> {
        self.git(&["add", ".aether/"])?;
        Ok(())
    }
    ...
    /// Check if there are unstaged changes in .aether/
    fn has_unstaged_changes(&self) -> bool {
        self.git(&["diff", "--quiet", ".aether/"])
            .is_err()
    }
    ...
    pub fn commit_snapshot(&mut self, message: &str) -> Result<CommitHash> {
        ...
        if self.has_staged_changes() {
            let full_message = format!("aether: {}", message);
    ...
    pub fn restore_snapshot(&mut self, commit: &CommitHash) -> Result<()> {
        ...
        // Checkout .aether/ from specific commit
        self.git(&["checkout", commit, "--", ".aether/"])?;
        
        // Reload store
        self.store = MemoryStore::new(Some(self.aether_path.join("memory.toml")))?;
        
        Ok(())
    }
    
    /// List all snapshots (commits touching .aether/)
...
mod tests {
    ...
    fn test_snapshot_info_parse() {
        let info = SnapshotInfo {
            ...
            message: "aether: snapshot".to_string(),
