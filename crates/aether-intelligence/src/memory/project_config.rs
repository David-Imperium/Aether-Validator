//! Project Config - `.aether.toml` file parsing and merge
//!
//! This module handles the user-editable project configuration file.
//! It's loaded from `.aether.toml` in the project root and merged with
//! the learned config from memory.
//!
//! ## File Location
//!
//! - `<project_root>/.aether.toml` - Project-specific config
//! - `<project_root>/.aether/config.toml` - Alternative location (preferred for multi-file)
//!
//! ## Example .aether.toml
//!
//! ```toml
...
/// Project configuration loaded from `.aether.toml`
...
/// Dubbioso Mode section in .aether.toml
///
...
impl From<DubbiosoSection> for crate::dubbioso::DubbiosoConfig {
    fn from(section: DubbiosoSection) -> Self {
        // Use explicit values from .aether.toml
...
/// Whitelist section in .aether.toml
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// A single whitelist entry from .aether.toml
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// A custom rule from .aether.toml
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl ProjectConfig {
    /// Load project config from `.aether.toml` in project root
    pub fn load(project_root: &Path) -> Result<Option<Self>> {
        // Try primary location
        let config_path = project_root.join(".aether.toml");
        
        // Try alternative location
        let alt_path = project_root.join(".aether").join("config.toml");
    ...
    /// Save project config to `.aether.toml`
