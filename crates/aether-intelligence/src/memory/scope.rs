...
//!
//! Aether uses a hybrid memory architecture:
//! - Global memory: ~/.aether/global/ (universal patterns, user preferences)
//! - Project memory: <project>/.aether/ (project-specific decisions, git-trackable)
...
impl MemoryPath {
    ...
    /// Global base path: ~/.aether/
    ///
    /// Uses home directory for user-accessible storage:
    /// - Windows: C:\Users\<user>\.aether\
    /// - Linux/macOS: ~/.aether/
...
"#;
    ...
    /// Copy README template to project .aether/
    pub fn ensure_readme(&self) -> Result<()> {
        match &self.scope {
            ...
            MemoryScope::Project { .. } => {
                ...
                if !readme.exists() {
                    let content = include_str!("templates/aether-readme.md");
...
mod tests {
    ...
    fn test_global_paths() {
        ...
        // base() returns ~/.aether/ for Global scope (NOT ~/.aether/global/)
        // Use Path::new for cross-platform compatibility
        assert!(path.base().ends_with(std::path::Path::new(".aether")));
