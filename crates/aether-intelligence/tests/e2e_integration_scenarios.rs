...
//! 2. Global memory - verify ~/.aether/global/ updates
//! 3. Local memory without git - creates .aether/ directory
...
// ============================================================================
// TEST 2: Global Memory - ~/.aether/global/ Updates
// ============================================================================
...
fn test_global_memory_path_resolution() {
    ...
    let base = path.base();
    assert!(base.to_string_lossy().contains(".aether"), 
            "Global path should contain .aether");
...
// ============================================================================
// TEST 3: Local Memory Without Git - Creates .aether/ Directory
// ============================================================================

/// Test: Creates .aether/ for non-git project
#[test]
fn test_creates_aether_dir_without_git() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    ...
    let base = path.base();
    assert!(base.ends_with(".aether"), "Base should be .aether directory");
    
    // Initialize memory directory
    path.ensure_dirs().expect("Failed to create directories");
    
    assert!(base.exists(), ".aether directory should be created");
...
fn test_local_memory_project_patterns() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let memory_path = temp_dir.path().join(".aether");
