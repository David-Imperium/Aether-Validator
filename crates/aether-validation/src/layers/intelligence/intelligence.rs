//! Intelligence Layer — Integrates Aether Intelligence 5-layer system
//!
//! This layer wraps `AetherIntelligence` to provide:
...
//! ## Feature Flags
//!
//! This layer requires `aether-intelligence` dependency.
...
/// Intelligence Layer that integrates Aether Intelligence 5-layer system
///
/// When `aether-intelligence` is not available, this layer passes through
...
impl ValidationLayer for IntelligenceLayer {
    ...
    async fn validate(&self, _ctx: &ValidationContext) -> LayerResult {
        ...
        if self.config.patterns {
            use aether_intelligence::{FeatureExtractor, AnomalyDetector};
    ...
    #[cfg(not(feature = "aether-intelligence"))]
    async fn validate(&self, _ctx: &ValidationContext) -> LayerResult {
        LayerResult::pass()
