...
use synward_validation::{ValidationPipeline, SyntaxLayer, SemanticLayer, LogicLayer, SecurityLayer, FallbackSecurityLayer, ComplexityLayer, SupplyChainLayer, ValidationContext, PipelineResult, Violation, Severity, ScopeAnalysisLayer, TypeInferenceLayer, ValidationLayer};
use synward_certification::{Keypair, CertificateVerifier, Certificate, ValidationResult, AgentInfo, VerifyingKey};
...
/// Convert synward_validation::Severity to memory::Severity
#[cfg(feature = "intelligence")]
fn to_memory_severity(sev: Severity) -> synward_intelligence::memory::Severity {
    use synward_intelligence::memory::Severity as MemSev;
    match sev {
...
fn print_fallback_result(
    ...
    println!();
    println!("{}", "╔══════════════════════════════════════════════════════════════╗".cyan());
    println!("{} {}{}", "║".cyan(), format!("SYNWARD - Validating {} (Unknown - fallback mode)", path.display()).bold(), ".".cyan());
    println!("{}", "╠══════════════════════════════════════════════════════════════╣".cyan());
...
fn print_validation_result(
    ...
    println!();
    println!("{}", "╔══════════════════════════════════════════════════════════════╗".cyan());
    println!("{} {}{}", "║".cyan(), format!("SYNWARD - Validating {} ({})", path.display(), language).bold(), ".".cyan());
    println!("{}", "╠══════════════════════════════════════════════════════════════╣".cyan());
...
fn print_validation_result_with_dubbioso(
    ...
    dubbioso: bool,
    total_criticals: usize,
    dubbioso_results: &Option<Vec<synward_intelligence::dubbioso_validator::DubbiosoValidationResult>>,
...
// ============================================================================
// SELF VALIDATE (Synward validates itself)
// ============================================================================

pub async fn self_validate(severity: &str, format: &str) -> Result<()> {
    // Find Synward source directory
    let synward_dir = std::env::current_dir()?
        .ancestors()
        .find(|p| p.join("Cargo.toml").exists())
        .map(|p| p.to_path_buf())
        .ok_or_else(|| anyhow::anyhow!("Could not find Synward project directory"))?;

    // Collect all .rs files (exclude tests, fixtures, benchmarks, examples)
    let mut rust_files: Vec<PathBuf> = Vec::new();
    for entry in walkdir::WalkDir::new(&synward_dir)
        .into_iter()
        .filter_map(|e| e.ok())
    {
        let path = entry.path();
    ...
    for file in &rust_files {
        ...
        // Skip contract checks in self-validation - Synward defines these rules,
    ...
    if format == "json" {
        ...
        println!();
        println!("{}", "╔══════════════════════════════════════════════════════════════╗".cyan());
        println!("{} {}", "║".cyan(), "SYNWARD SELF-VALIDATION".bold());
        ...
        for (file, violations) in &file_results {
            let rel_path = file.strip_prefix(&synward_dir).unwrap_or(file);
        ...
        if total_errors == 0 && total_warnings == 0 {
            println!("{} {} {}", "║".cyan(), "✓".green(), "Synward code looks good!".green());
        } else if total_errors > 0 {
            println!("{} {} {}", "║".cyan(), "✗".red(), "Fix errors in Synward source code".red());
        } else {
            println!("{} {} {}", "║".cyan(), "⚠".yellow(), "Consider fixing warnings".yellow());
        }
...
pub async fn analyze(file: PathBuf, format: &str) -> Result<()> {
    let language = detect_language(&file)
        .ok_or_else(|| {
            ...
            anyhow::anyhow!("Language '.{}' not supported. Use 'synward validate' for fallback security checks.", ext)
        })?;
    ...
    // Count nodes
    fn count_nodes(node: &synward_parsers::ASTNode) -> usize {
        1 + node.children.iter().map(count_nodes).sum::<usize>()
    }
    fn max_depth(node: &synward_parsers::ASTNode) -> usize {
...
pub async fn init(lang: Option<String>, platform: Option<String>, level: Option<String>, config: Option<PathBuf>) -> Result<()> {
    println!();
    println!("{}", "╔═══════════════════════════════════════════════════════════════╗".cyan());
    println!("{} {}", "║".cyan(), "                     SYNWARD SETUP v0.1                          ".cyan());
    println!("{}", "╚═══════════════════════════════════════════════════════════════╝".cyan());
    println!();
    ...
    if selected_languages.is_empty() {
        ...
        println!("{}", "Usage: synward init --lang rust,python --platform vscode --level standard".yellow());
        std::process::exit(1);
    }
    ...
    println!("{}", "║".cyan());
    println!("{} To update: {}", "║".cyan(), "synward contracts update".yellow());
    println!("{} To check: {}", "║".cyan(), "synward contracts check".yellow());
    println!("{}", "╚═══════════════════════════════════════════════════════════════╝".cyan());
...
pub async fn contracts_check() -> Result<()> {
    ...
    println!("{}", "╚══════════════════════════════════════════════════════════════╝".cyan());
    println!();
    println!("Run {} to update contracts", "synward contracts update".yellow());
