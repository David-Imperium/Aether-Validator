//! Git hooks management for Synward
//!
//! Installs git hooks to integrate Synward validation into the git workflow.
...
/// Hook types supported by Synward
...
pub fn uninstall_hooks(project_path: &PathBuf, hooks: &[HookType]) -> Result<()> {
    ...
    for hook_type in hooks {
        ...
        if hook_path.exists() {
            // Check if it's an Synward hook
            let content = fs::read_to_string(&hook_path)?;
            if content.contains("SYNWARD_HOOK") {
                ...
                println!("Skipping {} - not an Synward hook", hook_type);
            }
        }
    }
...
fn generate_pre_commit_hook(config: &HookConfig) -> String {
    ...
    format!(r#"#!/bin/sh
# SYNWARD_HOOK: pre-commit
# Generated by Synward - Do not edit manually
...
echo "Synward: Validating staged files..."

# Run Synward validation on staged files
echo "$STAGED_FILES" | xargs synward validate{} --severity {}

EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    echo ""
    echo "Synward validation failed. Commit blocked."
    echo "Fix the issues or use 'git commit --no-verify' to bypass."
    exit 1
fi

echo "Synward validation passed!"
exit 0
...
fn generate_post_commit_hook(config: &HookConfig) -> String {
    ...
    format!(r#"#!/bin/sh
# SYNWARD_HOOK: post-commit
# Generated by Synward - Do not edit manually

echo "Synward: Updating memory with commit changes..."
...
# Update Synward memory
echo "$CHANGED_FILES" | xargs synward validate{}

echo "Synward memory updated!"
exit 0
"#, memory_flag)
}
...
fn generate_pre_push_hook(config: &HookConfig) -> String {
    if !config.pre_push_enabled {
        return r#"#!/bin/sh
# SYNWARD_HOOK: pre-push
# Generated by Synward - Do not edit manually
# DISABLED - Enable with --enable-pre-push flag

echo "Synward pre-push hook is disabled"
exit 0
"#.to_string();
    }
...
echo "Synward: Running full validation before push..."

# Validate entire project
synward validate . --severity {}

EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    echo ""
    echo "Synward validation failed. Push blocked."
    echo "Fix the issues or use 'git push --no-verify' to bypass."
    exit 1
fi

echo "Synward validation passed! Proceeding with push."
exit 0
"#, config.severity)
}
