...
use synward_validation::{ValidationPipeline, ValidationContext, Violation};
use synward_validation::layers::{SyntaxLayer, SemanticLayer, LogicLayer, SecurityLayer, ComplexityLayer, SupplyChainLayer, StyleLayer, ClippyLayer, ContractLayer};
use synward_validation::ScopeAnalysisLayer;
...
pub async fn run(args: ValidateArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let errors = result.results.iter()
        ...
        .filter(|v| v.severity == synward_validation::Severity::Error)
        .count();
    let warnings = result.results.iter()
        .flat_map(|(_, r)| &r.violations)
        .filter(|v| v.severity == synward_validation::Severity::Warning)
        .count();
    ...
    for (layer_name, layer_result) in &result.results {
        for violation in &layer_result.violations {
            let severity = match violation.severity {
                synward_validation::Severity::Error => "ERROR",
                synward_validation::Severity::Warning => "WARN",
                synward_validation::Severity::Info => "INFO",
                synward_validation::Severity::Hint => "HINT",
            };
