...
pub fn generate_config(platform: &str, languages: &[String], level: &str, cwd: &Path) -> Result<()> {
    ...
    match normalized {
        ...
        _ => {
            crate::ui::print_warning(&format!("Unknown platform '{}', creating synward.yaml", platform));
...
# Synward validation script for Claude Code / Droid
synward validate $args --contracts .factory/contracts
"#;
...
fn generate_vscode_config(languages: &[String], level: &str, cwd: &Path) -> Result<()> {
    ...
    let tasks = serde_json::json!({
        "version": "2.0.0",
        "tasks": [{
            "label": "Synward: Validate",
            "type": "shell",
            "command": "synward validate ${file} --contracts .factory/contracts",
            "problemMatcher": []
        }]
    });
...
fn generate_neovim_config(languages: &[String], level: &str, cwd: &Path) -> Result<()> {
    let lua_dir = cwd.join("lua").join("synward");
    ...
    let config = format!(r#"
-- Synward configuration for Neovim
-- Level: {}
-- Languages: {}
...
vim.api.nvim_create_autocmd("BufWritePost", {{
    pattern = {{{}}},
    callback = function()
        vim.fn.system("synward validate " .. vim.fn.expand("%:p") .. " --contracts .factory/contracts")
    end,
}})
...
"#, level, lang_list, lang_list, level, lang_patterns.join(", "));
    ...
    crate::ui::print_file_created("lua/synward/init.lua");
...
<project version="4">
  <component name="SynwardSettings">
...
"#, languages.join(","), level);
    ...
    fs::write(idea.join("synward.xml"), xml)?;
    crate::ui::print_file_created(".idea/synward.xml");
...
fn generate_gemini_config(languages: &[String], level: &str, cwd: &Path) -> Result<()> {
    ...
    let config = serde_yaml::to_string(&serde_yaml::Value::Mapping(
        serde_yaml::Mapping::from_iter(vec![
            ...
            (serde_yaml::Value::String("tool".to_string()), serde_yaml::Value::String("synward validate".to_string())),
        ])
    ))?;

    fs::write(cwd.join("synward.yaml"), config)?;
    crate::ui::print_file_created("synward.yaml");
