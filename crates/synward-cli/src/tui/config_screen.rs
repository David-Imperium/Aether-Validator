//! Configuration Screen - Edit .synward.toml

use std::path::PathBuf;
use synward_intelligence::memory::ProjectConfig;
use synward_intelligence::DubbiosoPreset;
...
impl ConfigScreen {
    ...
    pub fn apply_dubbioso_preset(&mut self) {
        if let Some(config) = &mut self.config {
            ...
            // Apply preset values
            let preset_config = synward_intelligence::DubbiosoConfig::from(preset);
