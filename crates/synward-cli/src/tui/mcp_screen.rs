...
pub enum PlatformStatus {
    /// Platform installed, synward MCP already configured
    Configured,
    /// Platform installed, synward MCP not yet configured
    Detected,
...
impl McpScreen {
    ...
    pub fn generate_config(&self) -> Result<String, String> {
        ...
        let synward_mcp_path = find_synward_mcp_binary();
...
}}"#, platform.config_path.display(), synward_mcp_path))
            }
        ...
        match platform.config_format {
            ConfigFormat::ZedContextServers => {
                ...
                ctx.as_object_mut()
                    ...
                    .insert("synward".to_string(), serde_json::json!({
                        "command": { "path": synward_mcp_path, "args": [] }
                    }));
...
fn detect_platforms(project_root: &Path) -> Vec<DetectedPlatform> {
    ...
    // VS Code (Copilot MCP) — .vscode/mcp.json
    {
        ...
        let has_synward = has_synward_in_json(&config_path, "mcpServers");
        platforms.push(DetectedPlatform {
            name: "VS Code (Copilot)",
            status: if has_synward { PlatformStatus::Configured } else if installed { PlatformStatus::Detected } else { PlatformStatus::NotFound },
            config_path,
    ...
    // Zed — ~/.config/zed/settings.json
    {
        ...
        let has_synward = has_synward_in_json(&config_path, "context_servers");
        platforms.push(DetectedPlatform {
            name: "Zed",
            status: if has_synward { PlatformStatus::Configured } else if installed { PlatformStatus::Detected } else { PlatformStatus::NotFound },
            config_path,
    ...
    // BeeAI Framework — ~/.beeai/config.yaml (agentic framework)
    {
        ...
        // BeeAI uses YAML, check for synward reference
        let has_synward = if config_path.exists() {
            fs::read_to_string(&config_path).ok()
                .map(|s| s.contains("synward"))
                .unwrap_or(false)
        } else { false };
        platforms.push(DetectedPlatform {
...
/// Check if a JSON config file has "synward" under a given key
fn has_synward_in_json(path: &Path, key: &str) -> bool {
    if !path.exists() {
        return false;
    }
    ...
    json.get(key)
        ...
        .map(|obj| obj.contains_key("synward"))
        .unwrap_or(false)
}

fn find_synward_mcp_binary() -> String {
    if let Ok(path) = which::which("synward-mcp") {
        return path.to_string_lossy().to_string();
    }
    if let Ok(exe) = std::env::current_exe() {
        let dir = exe.parent().unwrap_or(Path::new("."));
        let candidate = dir.join("synward-mcp");
        if candidate.exists() {
            return candidate.to_string_lossy().to_string();
        }
        #[cfg(windows)]
        {
            let candidate = dir.join("synward-mcp.exe");
            if candidate.exists() {
                return candidate.to_string_lossy().to_string();
            }
        }
    }
    "synward-mcp".to_string()
}
