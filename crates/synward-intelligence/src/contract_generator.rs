...
//!
//! Questo modulo genera file YAML di contract Synward a partire dai
//! pattern estratti dal PatternLearner.
//!
//! ## Panoramica
//!
//! Quando `synward learn` estrae convenzioni da un progetto, questo modulo
...
//! use synward_intelligence::contract_generator::ContractGenerator;
//!
...
//! generator.save("./.synward/contracts/learned.yaml")?;
//! ```
...
impl ContractGenerator {
    ...
    /// Converte in formato YAML per Synward contracts
    ...
    /// Genera contract in formato Synward nativo
    pub fn to_synward_contract(&self) -> SynwardContract {
        SynwardContract {
            api_version: "synward.dev/v1".into(),
            kind: "NamingContract".into(),
            metadata: SynwardContractMeta {
                name: format!("{}-learned", self.project),
                learned: true,
            },
            spec: SynwardContractSpec {
                rules: self.naming_rules.iter()
                    .map(|r| SynwardContractRule {
...
/// Contract formato nativo Synward
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SynwardContract {
    pub api_version: String,
    pub kind: String,
    pub metadata: SynwardContractMeta,
    pub spec: SynwardContractSpec,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SynwardContractMeta {
    pub name: String,
    pub learned: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SynwardContractSpec {
    pub rules: Vec<SynwardContractRule>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SynwardContractRule {
    pub pattern: String,
    pub allowed: Vec<String>,
    pub confidence: f64,
}
...
mod tests {
    ...
    fn test_generator_creation() {
        ...
        assert_eq!(generator.metadata.source, "synward-learn");
    }
