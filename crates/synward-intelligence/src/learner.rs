...
//! da un progetto esistente, permettendo ad Synward di adattarsi allo stile del progetto.
//!
//! ## Panoramica
//!
...
//! ## Output
//!
//! Produce un file TOML (`.synward/learned.toml`) con tutte le convenzioni scoperte:
//!
//! ```toml
//! project = "my-project"
//! language = "rust"
...
//!
//! ```rust,ignore
//! use synward_intelligence::learner::PatternLearner;
//!
...
//! ```
//!
//! ## Integrazione con Synward
//!
//! Questo modulo è utilizzato dal comando `synward learn` per personalizzare
...
/// durante l'analisi. Viene serializzato in TOML e salvato in `.synward/learned.toml`.
///
/// ## Campi Principali
///
...
///
/// ```rust,ignore
/// use synward_intelligence::learner::{PatternLearner, LearnedPatterns};
///
/// let mut learner = PatternLearner::new("my-project");
/// learner.analyze_file(source)?;
...
///
/// Quando un nuovo struct viene creato, Synward può suggerire
/// suffissi basati su questi pattern:
///
/// ```rust
/// // Se "Config" è il suffisso più comune
/// // Synward suggerisce: AppConfig, DatabaseConfig, ecc.
/// ```
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Synward può avvertire quando un nuovo struct non li deriva.
///
/// ```rust
...
pub struct DerivePatterns {
    ...
    /// Synward può avvertire se un nuovo struct pubblico non lo deriva.
...
/// ## Utilizzo nei Controlli
///
/// Se `public_doc_percentage` è alto (> 80%), Synward può avvertire
...
/// ## Utilizzo nei Controlli
///
/// Se `wildcard_usage_percentage` è basso (< 10%), Synward può
...
pub struct ImportPatterns {
    ...
    /// import espliciti. Synward può avvertire sui wildcard.
...
/// let toml = learner.to_toml()?;
/// std::fs::write(".synward/learned.toml", toml)?;
/// # Ok::<(), Box<dyn std::error::Error>>(())
/// ```
///
/// ## Thread Safety
...
/// ## Integrazione CLI
///
/// Usato dal comando `synward learn`:
///
/// ```bash
/// synward learn --project ./my-project --output .synward/learned.toml
/// ```
...
impl PatternLearner {
    ...
    /// che può essere riutilizzato da Synward per la validazione.
    ///
    /// # Returns
    ///
