...
mod tests {
    ...
    fn fresh_drift_store() -> DriftSnapshotStore {
        let temp_path = std::env::temp_dir().join(format!("synward_drift_{}_{}.json", std::process::id(), chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)));
...
"#;
    ...
    // =========================================================================
    // Tests that verify Synward can detect:
    ...
    fn build_snapshot_store_with_drift(
        ...
        let temp_path = std::env::temp_dir().join(format!("synward_drift_snapshots_{}_{}.json", std::process::id(), chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)));
