...
mod tests {
    ...
    fn test_decision_log_roundtrip() {
        ...
        let temp_path = temp_dir().join(format!("synward_test_decisions_{}.json", std::process::id()));
    ...
    fn test_semantic_recall() {
        ...
        let temp_path = temp_dir.join("synward_test_decisions_semantic.json");
    ...
    fn test_supersede_decision() {
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join("synward_test_supersede.json");
        let mut log = DecisionLog::new(Some(temp_path)).unwrap();
        log.clear().unwrap();
    ...
    fn test_supersede_chain() {
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join("synward_test_chain.json");
        let mut log = DecisionLog::new(Some(temp_path)).unwrap();
        log.clear().unwrap();
    ...
    fn test_recall_ranked() {
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join("synward_test_ranked.json");
        let mut log = DecisionLog::new(Some(temp_path)).unwrap();
        log.clear().unwrap();
