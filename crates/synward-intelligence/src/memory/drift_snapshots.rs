...
mod tests {
    ...
    fn test_record_and_analyze() {
        ...
        let temp_path = temp_dir.join("synward_test_drift.json");
    ...
    fn test_analyze_days() {
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join("synward_test_drift_days.json");
        let mut store = DriftSnapshotStore::new(Some(temp_path.clone())).unwrap();
    ...
    fn test_alerts() {
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join("synward_test_drift_alerts.json");
        let mut store = DriftSnapshotStore::new(Some(temp_path.clone())).unwrap();
