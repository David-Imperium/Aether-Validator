//! Config Resources - Expose Synward configuration via MCP
...
/// Synward configuration resource
...
pub fn list_config_resources() -> Vec<RawResource> {
    vec![
        RawResource::new(
            "synward://config/main".to_string(),
            "Synward Configuration".to_string(),
        ),
        RawResource::new(
            "synward://config/compliance".to_string(),
            "Compliance Engine Configuration".to_string(),
        ),
        RawResource::new(
            "synward://config/dubbioso".to_string(),
...
pub fn read_config_resource(uri: &str) -> Result<ConfigResource> {
    // Default configuration (would load from .synward/config.toml if exists)
    Ok(ConfigResource {
