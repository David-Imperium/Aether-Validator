//! Synward Client — High-level client for validation and certification
...
use synward_certification::{Certificate, Keypair};
use synward_validation::{ValidationPipeline, ValidationContext};
use synward_validation::layers::{SyntaxLayer, SemanticLayer, LogicLayer};
use synward_parsers::{RustParser, Parser, AST};

use base64::{Engine as _, engine::general_purpose::STANDARD as BASE64};
use std::path::Path;
use std::sync::Arc;

/// Synward client for validation and certification
pub struct SynwardClient {
    /// Validation pipeline
...
impl SynwardClient {
    /// Create a new Synward client with default configuration
    pub fn new() -> Self {
        Self {
            pipeline: Arc::new(
...
fn count_nodes(node: &synward_parsers::ASTNode, stats: &mut crate::types::AstStats) {
...
impl Default for SynwardClient {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_client_creation() {
        let client = SynwardClient::new();
        assert!(client.signer.is_none());
    }

    #[test]
    fn test_client_with_signer() {
        let keypair = SynwardClient::generate_keypair();
        let client = SynwardClient::new()
