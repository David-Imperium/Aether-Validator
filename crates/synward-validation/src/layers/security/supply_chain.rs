...
impl SupplyChainLayer {
    ...
    fn check_source_patterns(&self, ctx: &ValidationContext, violations: &mut Vec<Violation>) {
        for (line_num, line) in ctx.source.lines().enumerate() {
            ...
            // Skip if this is pattern definition code (contains .contains("curl") etc.)
            let is_detection_code = line.contains(".contains(") 
                || line.contains(".contains('")
                || line.contains("check_source_patterns")
                || line.contains("SUPP006");
            
            if !is_detection_code && (line.contains("curl") || line.contains("wget")) && line.contains("|") && line.contains("bash") {
                violations.push(Violation {
