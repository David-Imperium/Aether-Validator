# Aether Neural Native — Design Document
...
Aether evolve da validatore rule-based a **sistema neuro-symbolic nativo**. Le regole e i contratti rimangono il layer simbolico, ma una rete neurale custom fornisce reasoning, pattern discovery, predizione drift e suggerimenti fix — tutto senza dipendere da LLM esterni (locali o cloud).
...
| **Parametri** | ~8M |
| **Training** | Supervisionato su risultati validazione Aether + dataset esterni |

**Pipeline**:
```
Codice sorgente
    → Tree-sitter (parsing multi-linguaggio, già in Aether)
    → AST
...
AST  → Struttura sintattica (già in Aether via tree-sitter)
...
- `violations_count`: violazioni Aether storiche in questa posizione
...
Aether genera automaticamente training data ad ogni validazione:

```toml
# .aether/training_data/val_20260330_001.toml
[source]
...
NexusTrain diventa il training hub per tutte le reti di Aether.
...
│   │   ├── aether_loader.py   #    Loader da .aether/training_data/
...
├── configs/
│   ├── aether_gnn.yaml        # NUOVO — config Code Reasoner
│   ├── aether_pattern.yaml    # NUOVO — config Pattern Memory
│   └── aether_drift.yaml      # NUOVO — config Drift Predictor
```

### 5.2 Config di Esempio (Code Reasoner)

```yaml
# configs/aether_gnn.yaml
model:
...
data:
  sources:
    - type: "aether_validation"  # Da .aether/training_data/
      path: ".aether/training_data/"
...
Validazione Aether
    ...
    → Scritto in .aether/training_data/
...
**Temporal Memory** (già esistente in Aether) traccia le versioni dei modelli:
...
## 6. Runtime (Burn in Aether)

### 6.1 Crate Structure

```
crates/aether-neural/          # NUOVO crate
├── Cargo.toml
├── src/
│   ├── lib.rs                  # API pubblica
...
```rust
/// Aether Neural — Layer neurale per Aether
...
### 6.3 Integration con Aether esistente

```rust
// In aether-intelligence/src/lib.rs — estensione

impl AetherIntelligence {
    /// Nuovo: accesso al layer neurale
    pub fn neural(&self) -> Option<&AetherNeural>;
...
| Dataset iniziale | Generare training data dalle validazioni Aether esistenti + corpus esterno |
...
| Burn inference base | `aether-neural` crate con caricamento modello ONNX |
| Test end-to-end | Codice → CPG → GNN → classificazione → risultato |

**Deliverable**: Aether rileva problemi con il GNN, oltre alle regole.
...
**Deliverable**: Aether riconosce pattern ricorrenti e recupera esperienze simili.
...
**Deliverable**: Aether prevede drift e segnala trend negativi.
...
**Deliverable**: Aether fornisce spiegazioni, suggerisce fix, migliora autonomamente.
...
| Task | Dettaglio |
|---|---|
| Contratti neuro-symbolic | I contratti Aether informano direttamente le reti |
...
| Documentazione completa | Aggiornamento di tutti i documenti Aether |

**Deliverable**: Aether è un sistema neuro-symbolic nativo, integrato e autonomo.
...
```toml
# crates/aether-neural/Cargo.toml
[dependencies]
...
# Riutilizzo da Aether esistente
...
| Aether DecisionLog | Domain-specific validation data | Cresce con l'uso |
...
| Documento | Relazione |
|---|---|
| [AETHER_INTELLIGENCE.md](AETHER_INTELLIGENCE.md) | Layer 3→5 diventano neuro-symbolic |
