/**
 * Aether Client - Communication with Aether CLI/MCP
 */
...
export class AetherClient {
    ...
    constructor() {
        this.config = vscode.workspace.getConfiguration('aether');
        this.aetherPath = this.config.get<string>('aetherPath') || 'aether';
    }
    ...
    async getComplianceStatus(): Promise<ComplianceStatus> {
        return new Promise((resolve, reject) => {
            exec(
                `"${this.aetherPath}" compliance status --json`,
                { maxBuffer: 1024 * 1024 },
    ...
    async acceptViolation(
        ...
        return new Promise((resolve, reject) => {
            exec(
                `"${this.aetherPath}" compliance accept "${ruleId}" "${filePath}" --reason "${reason}"`,
    ...
    async analyzeDrift(filePath: string, days: number = 30): Promise<DriftResult> {
        return new Promise((resolve, reject) => {
            exec(
                `"${this.aetherPath}" drift analyze "${filePath}" --days ${days} --json`,
                { maxBuffer: 1024 * 1024 },
                (error, stdout, _stderr) => {
                    if (error) {
                        // Return default if command fails
    ...
    async suggestFix(
        ...
        return new Promise((resolve, reject) => {
            const process = spawn(this.aetherPath, [
                'suggest',
