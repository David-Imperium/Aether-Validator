/**
 * Aether Code Actions - Quick fixes and refactorings
 */

import * as vscode from 'vscode';
import { AetherClient, Violation } from './aetherClient';
import { AetherDiagnostics } from './diagnostics';

export class AetherCodeActions implements vscode.CodeActionProvider {
    ...
    private diagnostics: AetherDiagnostics;
    
    constructor(client: AetherClient, diagnostics: AetherDiagnostics) {
        this.client = client;
    ...
    async provideCodeActions(
        ...
        const aetherDiagnostics = context.diagnostics.filter(
            d => d.source === 'Aether'
        );
        
        for (const diagnostic of aetherDiagnostics) {
            ...
            acceptAction.command = {
                command: 'aether.acceptViolation',
    ...
    private createSuppressEdit(
        ...
        if (scope === 'file') {
            ...
            const suppression = `// aether:suppress ${violationId}\n`;
            edit.insert(
                document.uri,
            ...
            const suppression = `${indentation}// aether:ignore ${violationId}\n`;
            edit.insert(
                document.uri,
