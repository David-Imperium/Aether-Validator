...
import { AetherDiagnostics } from './diagnostics';
import { AetherCodeActions } from './codeActions';
import { StatusBarManager } from './statusBar';
import { AetherPanel } from './panel';

let client: AetherClient;
let diagnostics: AetherDiagnostics;
let statusBar: StatusBarManager;

export async function activate(context: vscode.ExtensionContext) {
    console.log('Aether extension is activating...');

    // Initialize client
    client = new AetherClient();
    
    // Initialize diagnostics
    diagnostics = new AetherDiagnostics();
    
    // Initialize status bar
    statusBar = new StatusBarManager();
    
    // Register code actions provider
    const codeActions = new AetherCodeActions(client, diagnostics);
    ...
    for (const lang of supportedLanguages) {
        context.subscriptions.push(
            vscode.languages.registerCodeActionsProvider(
                ...
                codeActions,
                { providedCodeActionKinds: AetherCodeActions.providedKinds }
            )
        );
    }
    ...
    console.log('Aether extension activated successfully');
}
...
function registerCommands(
    ...
    // Open Aether Panel (main UI)
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.openPanel', () => {
            AetherPanel.createOrShow(context.extensionUri, client);
        })
    );
    
    // Validate current file
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.validate', async () => {
            const editor = vscode.window.activeTextEditor;
            if (editor) {
                await validateDocument(editor.document, client, diagnostics, statusBar, context);
                vscode.window.showInformationMessage('Aether: Validation complete');
            }
        })
    );
    ...
    // Validate project
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.validateProject', async () => {
            const workspaceFolders = vscode.workspace.workspaceFolders;
            if (workspaceFolders) {
                vscode.window.withProgress({
                    location: vscode.ProgressLocation.Notification,
                    title: "Aether: Validating project...",
                    cancellable: false
                }, async (progress) => {
    ...
    // Accept violation
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.acceptViolation', async (violationId: string) => {
            ...
            if (reason) {
                ...
                if (editor) {
                    ...
                    vscode.window.showInformationMessage(`Aether: Violation ${violationId} accepted`);
                    // Re-validate
                    await validateDocument(editor.document, client, diagnostics, statusBar, context);
                }
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.showCompliance', async () => {
            AetherPanel.createOrShow(context.extensionUri, client);
            // Trigger compliance view
            setTimeout(() => {
                const panel = AetherPanel.currentPanel;
                if (panel) {
    ...
    // Analyze drift
    context.subscriptions.push(
        vscode.commands.registerCommand('aether.analyzeDrift', async () => {
            const editor = vscode.window.activeTextEditor;
            if (editor) {
                AetherPanel.createOrShow(context.extensionUri, client);
...
async function validateDocument(
    ...
    try {
        ...
        if (context && AetherPanel.currentPanel) {
            AetherPanel.currentPanel.updateViolations(
                document.uri.fsPath,
                result.violations,
                result.qualityScore
            );
        ...
        console.error('Aether validation error:', error);
...
async function validateProject(
    ...
    vscode.window.showInformationMessage(
        `Aether: Project validated. ${totalViolations} violations found. Quality: ${avgQuality.toFixed(0)}%`
    );
}
...
<head>
    <meta charset="UTF-8">
    <title>Aether Compliance Status</title>
    <style>
...
<head>
    <meta charset="UTF-8">
    <title>Aether Drift Analysis</title>
    <style>
        body { font-family: var(--vscode-font-family); padding: 20px; }
        h1 { color: var(--vscode-editor-foreground); }
...
export function deactivate() {
    console.log('Aether extension deactivated');
    if (client) {
        client.dispose();
    }
}
